...
import {
    CollectionPanel,
    type CollectionItem,
} from "./components/CollectionPanel/CollectionPanel";
...
export function Game({ onBack, isPaused }: Props) {
    ...
    const [selectedWaifuDetail, setSelectedWaifuDetail] =
        useState<Waifu | null>(null);
        ...
        setOwnedWaifus((prev) =>
            prev.map((waifu) => {
                const updated = new Waifu({
                    ...
                    baseStats: {
                        clickPower:
                            waifu.baseClickPower +
                            (type === "click_power" ? 1 : 0),
                        autoClick:
                            waifu.baseAutoClick +
                            (type === "auto_click" ? 0.5 : 0),
                        critChance:
                            waifu.baseCritChance +
                            (type === "crit_chance" ? 0.05 : 0),
    ...
    const handleSummon = (waifu: Waifu, cost: number, isDuplicate: boolean) => {
        ...
        if (isDuplicate) {
            setOwnedWaifus((prev) =>
                prev.map((w) => (w.id === waifu.id ? waifu : w)),
            );
    ...
    return (
        <div className="game">
            ...
            <nav className="game-sidebar">
                ...
                <button
                    className="side-btn"
                    onClick={() => setShowWaifuSelect(true)}
                >
                    <span>👩</span>
            ...
            <UpgradePanel
                ...
                upgradeLevels={{
                    ...
                    critChance: Math.floor(
                        globalUpgrades.critChanceBonus / 0.05,
                    ),
                }}
            />
