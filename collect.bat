...
:: ==========================================
:: СОДЕРЖИМОЕ ФАЙЛОВ (.ts / .tsx / .css)
:: ==========================================
echo ===== СОДЕРЖИМОЕ ФАЙЛОВ (.ts / .tsx / .css) ===== >> %OUTPUT%
echo. >> %OUTPUT%
...
:: --- 3. НОВОЕ: Собираем все .css (и .scss / .less) файлы ---
for /r %SOURCE_DIR% %%f in (*.css *.scss *.less) do (
    echo %%f | findstr /i "\\node_modules\\" >nul
    if errorlevel 1 (
        echo ===== FILE: %%f ===== >> %OUTPUT%
        type "%%f" >> %OUTPUT%
